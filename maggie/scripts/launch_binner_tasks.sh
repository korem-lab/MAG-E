            ...
            for t in $targets; do 
              for d in $modes; do
                ...
                if [ $? -eq 1 ]; then
                    ...
                    ASM=$a AOPT=$aopt MAP=$m MOPT=$mopt BOPT=$bopt TARGET=$t MODE=$d BIN=$b sbatch --job-name=$b --export=ALL,MAMBA_NO_LOCK=1,CONDA_NO_LOCK=1 binning/run_binner_gpu.sh
		              else
                    echo ASM=$a AOPT=$aopt MAP=$m MOPT=$mopt BOPT=$bopt TARGET=$t MODE=$d BIN=$b sbatch --export=ALL binning/run_binner.sh
                    ASM=$a AOPT=$aopt MAP=$m MOPT=$mopt BOPT=$bopt TARGET=$t MODE=$d BIN=$b sbatch --job-name=$b --export=ALL binning/run_binner.sh
	                fi
                else
