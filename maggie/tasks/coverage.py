...
class Coverage(ABC):
    ...
    @abstractmethod
    def run_main(self, r1, r2, coverage_dir, target, cov_sample, threads, options, **kwargs):
        """
    ...
    @abstractmethod 
    def main_done(self, coverage_dir, target_sample, cov_sample, **kwargs):
        """
...
class bowtie2Coverage(Coverage):
    ...
    def run_main(self, simulation_dir, coverage_dir, cov_sample, threads, options, **kwargs):
...
class bowtie2JGICoverage(Coverage):
    ...
    def run_main(self, coverage_dir, target, **kwargs):
        # get to the coverage file 
        bams = glob(join(coverage_dir, '*.bam'))
        bams = ' '.join(bams)
        jgi = join(coverage_dir, 'coverage_mat_jgi.tsv')
        make_formats(bams, jgi, '')

        # single mode will be collected in same coverage_task directory
        bams = join(coverage_dir, f'{target}.bam')
        jgi = join(coverage_dir, 'target_coverage_mat_jgi.tsv')
        make_formats(bams, jgi, 'target_')

        # cleanup
        for f in glob(join(coverage_dir, '*.bam')):
            rm_file(f)
    ...
    def run_prep(self, coverage_dir, simulation_dir, samples, assembly_dir, threads, options, **kwargs):
        ...
        for s in samples:
            ...
            cmd = f'bowtie2 -p {threads} -u 100000 {options} -x {coverage_dir}/idx -1 {r1} -2 {r2} | samtools view -bS - > {bam}'
            run(cmd)
            sort_bam(bam, threads)
            index_bam(bam)

    def main_done(self, coverage_dir, samples, target, **kwargs):
        fls = [join(coverage_dir, 'coverage_mat_jgi.tsv'), join(coverage_dir, 'coverage_mat.tsv')] + \
              [join(coverage_dir, 'target_coverage_mat_jgi.tsv'), join(coverage_dir, 'target_coverage_mat.tsv')] + \
            [join(coverage_dir, f'{s}_coverage.tsv') for s in samples] + [join(coverage_dir, f'target_{target}_coverage.tsv')]
        return all(not_empty(e) for e in fls)

    def prep_done(self, coverage_dir, samples, **kwargs):
        return all(not_empty(join(coverage_dir, f'{s}.bam')) for s in samples)
...
class bwamem2JGICoverage(Coverage):
    ...
    def run_prep(self, coverage_dir, simulation_dir, samples, assembly_dir, threads, options, **kwargs):
        ...
        for s in samples:
            ...
            r2 = join(simulation_dir, f'{s}_R2.fastq.gz')
            bam = join(coverage_dir, f'{s}.bam')
            cmd = f'bwa-mem2 mem -t {threads} {options}  {coverage_dir}/idx {r1} {r2} | samtools view -bS - > {bam}'
            run(cmd)
            sort_bam(bam, threads)
            index_bam(bam)
...
class fairyCoverage(Coverage):
    ...
    def run_main(self, assembly_dir, coverage_dir, threads, target, **kwargs):
        ...
        jgi_to_vamb(coverage_dir, join(coverage_dir, 'target_coverage_mat_jgi.tsv'), 'target_')

    def run_prep(self, simulation_dir, coverage_dir, samples, threads, **kwargs):
        # make sketches
        for s in samples:
            r = join(simulation_dir, s)
            cmd = f'fairy sketch -t {threads} -1 {r}_R1.fastq.gz -2 {r}_R2.fastq.gz -S {s} -d {coverage_dir}'
            run(cmd)
    ...
    def prep_done(self, coverage_dir, samples, **kwargs):
        return all(not_empty(join(coverage_dir, f'{s}.paired.bcsp')) for s in samples)
...
class aembCoverage(Coverage):
    ...
    def run_prep(self, simulation_dir, assembly_dir, coverage_dir, samples, target, threads, **kwargs):
        ...
        for s in samples:
            r1 = join(simulation_dir, f'{s}_R1.fastq.gz')
            r2 = join(simulation_dir, f'{s}_R2.fastq.gz')
            cmd = f'{self.exec} -t {threads} --aemb {contigs} {r1} {r2} -o {coverage_dir}/{s}_coverage.tsv'
            run(cmd)
        covs = [join(coverage_dir, f'{s}_coverage.tsv') for s in samples]
        aembs_to_vamb(coverage_dir, covs)

        # single mode
        r1 = join(simulation_dir, f'{target}_R1.fastq.gz')
        r2 = join(simulation_dir, f'{target}_R2.fastq.gz')
        cmd = f'{self.exec} -t {threads} --aemb {contigs} {r1} {r2} -o {coverage_dir}/target_{target}_coverage.tsv'
        run(cmd)
        covs = [join(coverage_dir, f'{s}_coverage.tsv') for s in [target]]
        aembs_to_vamb(coverage_dir, covs,'target_')

        # semibin2 specific
        cmd = f'SemiBin2 split_contigs -i {contigs} -o {coverage_dir}'
        run(cmd)
        for s in samples:
            r1 = join(simulation_dir, f'{s}_R1.fastq.gz')
            r2 = join(simulation_dir, f'{s}_R2.fastq.gz')
            cmd = f'{self.exec} -t {threads} -R 6 --aemb {coverage_dir}/split_contigs.fna.gz {r1} {r2} -o {coverage_dir}/sb2_{s}_coverage.tsv'
            run(cmd)
    ...
    def prep_done(self, coverage_dir, samples, target, **kwargs):
        fls = [join(coverage_dir, 'coverage_mat.tsv'),join(coverage_dir, 'target_coverage_mat.tsv')] + \
            [join(coverage_dir, f'{s}_coverage.tsv') for s in samples] + [join(coverage_dir, f'target_{target}_coverage.tsv')]
        return all(not_empty(e) for e in fls)
