...
def build_database_table(database, genomes_dir, drep_dir, mage_db_dir):
    ...
    database.to_csv(join(mage_db_dir, 'metadata.csv'),index=None)
    return database

def construct_sylphdb(genome_list, mage_db_dir, db_prefix, c=200, t=8, force=False):
    os.makedirs(mage_db_dir,exist_ok=True)
    if not force and os.path.exists(f'{mage_db_dir}/{db_prefix}.syldb'):
        return
    with open(os.path.join(mage_db_dir, f'{db_prefix}.input_genome_list.txt'), 'w') as f:
        f.write('\n'.join(genome_list))
    run(f'sylph sketch -l {mage_db_dir}/{db_prefix}.input_genome_list.txt -o {mage_db_dir}/{db_prefix} -t {t} -c {c}', shell=True)